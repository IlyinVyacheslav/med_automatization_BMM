...
# Конфигурация подключения к PostgreSQL (берем из окружения Docker)
DB_CONFIG = {
    "host": os.getenv("DB_HOST", "127.0.0.1"),
    "database": os.getenv("DB_NAME", "clinic_bot_db"),
    "user": os.getenv("DB_USER", "ai_bot_registrar"),
    "password": os.getenv("DB_PASSWORD", "notahorse"),
...
def book_appointment(doctor_name, target_date, time_slot):
    """ЗАПИСЬ ЧЕРЕЗ ФУНКЦИЮ: Оформление талона с использованием clinic.book_nearest_slot."""
    ...
    try:
        ...
        # Находим ID специальности и точное время начала слота
        cur.execute("""
                    SELECT s.id, sl.starts_at
        ...
        specialty_id, starts_at = slot_data
        patient_id = st.session_state.active_patient["patient_id"]

        # Вызываем хранящуюся функцию из 01_init.sql с корректными параметрами
        cur.execute("""
            SELECT id FROM clinic.book_nearest_slot(%s, %s, %s, %s);
        """, (patient_id, specialty_id, starts_at, f"Запись через ИИ-бота к доктору {doctor_name}"))
...
# =====================================================================
# ИНТЕРФЕЙС STREAMLIT И ВЫЗОВ OLLAMA
# =====================================================================
...
if user_input := st.chat_input("Ваш запрос..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Анализ запроса..."):
            try:
                # Настройка клиента для связи с Ollama вне контейнера Docker
