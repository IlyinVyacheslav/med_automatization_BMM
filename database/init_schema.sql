...
-- ============================================================================
--  ОПЦИОНАЛЬНО: запись на КОНКРЕТНЫЙ слот (врач + дата + время).
--
--  Штатная модель бота — book_nearest_slot (ближайший слот специальности).
-- ============================================================================
CREATE OR REPLACE FUNCTION clinic.book_specific_slot(
    p_patient_id integer,
    p_slot_id    bigint,
    p_notes      text DEFAULT NULL
) RETURNS clinic.appointments
...
BEGIN
    ...
    -- блокируем выбранный слот; проверяем, что он открыт и свободен
    PERFORM 1
       FROM clinic.slots s
      WHERE s.id = p_slot_id AND s.is_available
      FOR UPDATE;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Слот id=% не найден или закрыт', p_slot_id
            USING ERRCODE = 'no_data_found';
    END IF;

    -- специальность берём как первую у врача этого слота
    SELECT ds.specialty_id INTO v_specialty_id
      FROM clinic.slots s
      JOIN clinic.doctor_specialties ds ON ds.doctor_id = s.doctor_id
    ...
    INSERT INTO clinic.appointments (slot_id, patient_id, specialty_id, status, notes)
    VALUES (p_slot_id, p_patient_id, v_specialty_id, 'booked', p_notes)
    RETURNING * INTO v_appt;      -- частичный уникальный индекс не даст двойной записи
    RETURN v_appt;

EXCEPTION WHEN unique_violation THEN
    RAISE EXCEPTION 'Слот id=% уже занят', p_slot_id
...
-- ============================================================================
--  ФУНКЦИЯ: запись «вне сетки» (создать слот при необходимости и оформить приём)
--  Нужна панели врача/регистратора: у registrar НЕТ прямого INSERT в slots,
--  поэтому слот создаётся под правами владельца функции (SECURITY DEFINER).
--  Переиспользует существующий слот в это же время, иначе создаёт новый.
-- ============================================================================
CREATE OR REPLACE FUNCTION clinic.book_adhoc(
    p_patient_id   integer,
    p_doctor_id    integer,
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM clinic.patients WHERE id = p_patient_id) THEN
        RAISE EXCEPTION 'Пациент id=% не найден', p_patient_id USING ERRCODE = 'foreign_key_violation';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM clinic.doctors WHERE id = p_doctor_id) THEN
        RAISE EXCEPTION 'Врач id=% не найден', p_doctor_id USING ERRCODE = 'foreign_key_violation';
    END IF;

    v_end := p_starts_at + make_interval(mins => p_duration_min);
    v_spec := COALESCE(
        p_specialty_id,
        (SELECT specialty_id FROM clinic.doctor_specialties
          WHERE doctor_id = p_doctor_id ORDER BY specialty_id LIMIT 1)
    );
    IF v_spec IS NULL THEN
        RAISE EXCEPTION 'У врача id=% не задана специальность', p_doctor_id USING ERRCODE = 'no_data_found';
    END IF;

    SELECT id INTO v_slot_id
      FROM clinic.slots
     WHERE doctor_id = p_doctor_id AND starts_at = p_starts_at
     FOR UPDATE;

    IF v_slot_id IS NULL THEN
        BEGIN
            INSERT INTO clinic.slots (doctor_id, starts_at, ends_at)
            VALUES (p_doctor_id, p_starts_at, v_end)
            RETURNING id INTO v_slot_id;
        EXCEPTION WHEN exclusion_violation THEN
            RAISE EXCEPTION 'Время % пересекается с другим слотом врача', p_starts_at
                USING ERRCODE = 'exclusion_violation';
        END;
    END IF;

    INSERT INTO clinic.appointments (slot_id, patient_id, specialty_id, status, notes)
    VALUES (v_slot_id, p_patient_id, v_spec, 'booked', p_notes)
    RETURNING * INTO v_appt;
    RETURN v_appt;

EXCEPTION WHEN unique_violation THEN
    RAISE EXCEPTION 'На этот слот уже есть активная запись' USING ERRCODE = 'unique_violation';
END;
$$;

-- ============================================================================
--  РОЛИ И ПРАВА
--  Групповые роли (NOLOGIN). Логин-учетки создаются отдельно и получают
--  членство: CREATE ROLE bot_svc LOGIN PASSWORD '...'; GRANT ai_bot_registrar TO bot_svc;
-- ============================================================================
DO $$ BEGIN CREATE ROLE ai_bot_registrar NOLOGIN; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE ROLE registrar        NOLOGIN; EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN CREATE ROLE admin            NOLOGIN; EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- убираем «широкие» дефолтные права
...
GRANT USAGE ON ALL SEQUENCES IN SCHEMA clinic TO registrar;  -- на будущее (SERIAL и т.п.)
-- НЕ выдаем: DELETE; изменение doctors/specialties/slots (расписание) — за admin.
...
GRANT SELECT ON clinic.patients, clinic.doctors, clinic.specialties,
    ...
    TO ai_bot_registrar;
GRANT INSERT ON clinic.patients TO ai_bot_registrar;   -- регистрация нового пациента
-- НЕ выдаем: UPDATE/DELETE нигде; прямой INSERT в appointments (только через функцию).

-- ---------- права на функции (по умолчанию EXECUTE у PUBLIC — забираем) ----------
...
REVOKE ALL ON FUNCTION clinic.book_specific_slot(integer,bigint,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION clinic.book_adhoc(integer,integer,timestamptz,integer,integer,text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION clinic.book_nearest_slot(integer,integer,timestamptz,text)
    TO ai_bot_registrar, registrar, admin;
GRANT EXECUTE ON FUNCTION clinic.book_specific_slot(integer,bigint,text)
    TO ai_bot_registrar, registrar, admin;
GRANT EXECUTE ON FUNCTION clinic.cancel_appointment(bigint,text)
    TO registrar, admin;                                -- боту отменять нельзя
GRANT EXECUTE ON FUNCTION clinic.generate_slots(integer,date,date,integer,time,time)
    TO admin;                                           -- расписание — только admin
GRANT EXECUTE ON FUNCTION clinic.book_adhoc(integer,integer,timestamptz,integer,integer,text)
    TO registrar, admin;