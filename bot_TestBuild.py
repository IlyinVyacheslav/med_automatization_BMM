...
DB_CONFIG = {
    ...
    "password": os.getenv("DB_PASSWORD", "ВАШ_ПАРОЛЬ_ОТ_ПОСТГРЕСА"),
...
def book_appointment(doctor_name, target_date, time_slot):
    ...
    try:
        ...
        # Вызываем хранимую функцию БД для записи (как и задумывалось изначально)
        cur.execute("""
    ...
    with st.chat_message("assistant"):
        with st.spinner("Анализ запроса..."):
            try:
                ...
                st.error(f"Ошибка выполнения: {e}")import streamlit as st
import psycopg2
import ollama
import json
import os
